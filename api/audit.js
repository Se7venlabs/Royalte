...
export default async function handler(req, res) {
  ...
  try {
    ...
    const auditCoverage = {
      ...
      appleMusic:    { status: appleMusicData.found ? 'Verified' : 'Not Connected', source: 'apple_music' },
...
async function getAppleMusic(artistName, isrc, spotifyTopTracks = []) {
  try {
    ...
    const artistResp = await fetch(`${BASE}/catalog/${STOREFRONT}/search?term=${encodeURIComponent(artistName)}&types=artists&limit=5`, { headers });
