...
### 2026-07-26 — Global Music Footprint™ — Canonical Artist Territory Intelligence™ — RATIFIED, MERGED

| | |
|---|---|
| **Date** | 2026-07-26 |
| **Decision** | Global Music Footprint™'s artist-only territory evaluation is corrected from a single, arbitrary catalog-order album to Canonical Artist Territory Intelligence™ — OR-aggregation over a Board-approved, Best Verified Release™-ranked sample of up to 5 releases. Approved for merge after satisfying all four parts of the Board's Pre-Merge Validation Directive (live Apple batch verification, explicit scope classification, live 4-artist regression, failure-state validation). |
| **Reason** | Resolves the Prince Territory Intelligence regression (Global Reach 1/167, Missing Markets 160/167, Distribution Health 1%) reported by the Board. Root cause, confirmed by direct code tracing: artist-only scans (no ISRC resolved) fell back to `extractFirstAlbumId()`, which selected Apple's arbitrary default-order first album, and Territory Intelligence Engine™ then correctly evaluated availability for that one wrong album across all 167 storefronts. Traced via git history to commit `584770d` (2026-07-02) as architecture drift predating the Territory Intelligence Engine by five weeks, not a deliberate design decision. The fix reuses two pieces of existing, Board-governed infrastructure — Best Verified Release™ scoring (ranking only) and the Engine's existing `data.length > 0` OR-style storefront classification (aggregation only) — rather than inventing new methodology. Territory Intelligence Engine™, Runtime Context, and Mission Control required and received zero changes; the correction is entirely contained within the acquisition layer. |
| **Impact** | `api/_lib/apple-pal-acquisition.js` (new `extractAlbumCandidates()`, `TERRITORY_SAMPLE_SIZE = 5`, artist-only AVAILABILITY selection rewired to `selectTopVerifiedReleases()`), `api/_lib/best-verified-release.js` (new export `selectTopVerifiedReleases()`, zero changes to existing Board-locked scoring), `provider-acquisition/connectors/apple-music/AppleMusicConnector.js` (`#fetchGlobalStorefrontAvailability()` now batches multiple album ids into one `ids=` request per storefront — same 167-request count as before). Methodology transparency added per the Board's Part 2 requirement: the 5-release sample is explicitly classified as a bounded acquisition approximation (not final canonical methodology), carried end-to-end as a `territoryMethodology`/`evaluationMethodology` object (`evaluationScope`/`sampleSize`/`catalogReleaseCount`/`selectionMethod`/`isCompleteCatalogEvaluation`) from the evidence package through to `api/_lib/territory-intelligence.js`'s output (`TERRITORY_INTELLIGENCE_VERSION` 1.1.0 → 1.2.0, additive only, no reconciliation-logic change); UI intentionally not wired to it yet, per the Board's own scope. Live verification performed against the PR #428 Preview deployment (real Apple + Spotify credentials, no secrets exposed to the working environment): Apple's `ids=` batch parameter confirmed to accept multiple IDs, match any of them, return HTTP 200 regardless of partial/invalid matches, and preserve comma semantics through URL-encoding. Live regression scans of Prince, Dua Lipa, Ada Lea, and Fela Kuti all returned 91–94% territory coverage (vs. Prince's original ~1%). All six Board-named failure scenarios (empty catalog, <5 eligible releases, invalid album id, rate-limited wave, timed-out storefront, partial 167-storefront completion) traced to code and confirmed to degrade to `UNKNOWN`/`NOT_EVALUATED`/`ERROR`, never fabricated `UNAVAILABLE`. Full detail: `governance/GLOBAL_MUSIC_FOOTPRINT_PRINCE_TERRITORY_REGRESSION_INVESTIGATION.md`, `GLOBAL_MUSIC_FOOTPRINT_ARTIST_LEVEL_TERRITORY_DESIGN_BRIEF.md`, `GLOBAL_MUSIC_FOOTPRINT_CANONICAL_ARTIST_TERRITORY_INTELLIGENCE_IMPLEMENTATION.md`, `GLOBAL_MUSIC_FOOTPRINT_PRE_MERGE_VALIDATION_REPORT.md`. |
| **Vote** | Board Approved |
| **PR Number** | #428 |
| **Commit SHA** | `1e7ed72` |
| **Test surface** | `tests/canonical-artist-territory-test.mjs` 12/12 (new file, up from 7 pre-validation-pass), `AppleMusicConnector.test.js` 51/51 (up from 49), `best-verified-release-test.mjs` 92/92, `canonical-scan-subject-test.mjs` 6/6 (2 stale assertions corrected to match production behavior), `pipeline-test.mjs` 230/230, `territory-scan-test.mjs` 31/31 — all green, zero regressions. |
