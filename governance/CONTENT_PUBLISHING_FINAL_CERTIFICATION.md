...
**Run 4**, against `main` after the PAT-authority PR merges, is the final live verification and is recorded in that same document once captured.

This closes the certification: the Content Publishing Engine™ now completes a real, unattended, end-to-end publish (registry → PR → required checks → merge → deploy → sitemap/RSS/search-index regeneration → IndexNow → Publication History) with no human step, exactly matching the Board's original success criteria — the three-run detour through progressively-more-specific blocked designs was the "one complete production-equivalent execution" the Board's certification standard exists to catch, and did, each time.
