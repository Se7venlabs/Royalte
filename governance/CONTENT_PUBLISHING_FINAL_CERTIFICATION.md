...
**Run 3** (`workflow_dispatch` after the `can_approve_pull_request_reviews` setting change and PR #456's merge): `gh pr create` succeeded — PR #457 opened as `github-actions[bot]`. But its required checks never ran: both `Run pipeline test` and `Validate content registry` sat at `action_required` with zero jobs created (confirmed via `gh api .../actions/runs/<id>` → `triggering_actor: github-actions[bot]`, empty `jobs` array). `gh pr checks --watch` returned early (no pending check-run object existed yet to watch), and the subsequent `gh pr merge` correctly failed on branch protection — no bypass occurred. PR #457 and its branch were closed/deleted; `main` was untouched. Root cause: GitHub gates `pull_request`-triggered workflow runs behind a manual "Approve and run" click whenever the PR was opened by `github-actions[bot]`, independent of any repo setting — this is GitHub enforcing almost exactly this program's own no-self-earned-checks governance rule, not a bug.

**Second resolution**: per the Executive Board's "Authentication & Automation Completion (Final Blocker Resolution)" directive (2026-08-03), a dedicated machine-account collaborator (`royalte-content-bot`, direct `write`-permission collaborator, not admin) was provisioned, and `scheduled-publish.yml` now authenticates via a fine-grained PAT (`CONTENT_PUBLISHING_PAT` repo secret) instead of `GITHUB_TOKEN`. A real collaborator identity is not subject to the bot-PR approval gate. Confirmed the org-level fine-grained-PAT restriction the directive asked to check for does not apply here — `Se7venlabs` is a personal user account, not an Organization. Full design, PAT permission minimum, and security validation are in `governance/CONTENT_PUBLISHING_AUTHENTICATION_MODEL.md` §4-5.

**Run 4** (`workflow_dispatch` on the PAT design, PR #460): authentication, branch, commit, push, and `gh pr create` all succeeded for the first time, and both required-and-relevant checks passed for real — but the workflow itself had already failed, one second after opening the PR, at `gh pr checks --watch --fail-fast`. Root cause: it polls once immediately and errors on "no checks reported" rather than waiting for GitHub to attach any — a startup race, not a governance or auth issue. PR #460 was completed with one manual `gh pr merge` once its checks had landed; the race itself was fixed in a follow-up commit (poll `commits/{sha}/check-runs` for existence, bounded, before handing off to `--watch`).

**Run 5** (`workflow_dispatch`, run `30855809849`, against `main` post-fix): fully unattended, zero manual steps. `publish.mjs` correctly retried the 4 already-scheduled-but-content-not-yet-merged articles (logged, not published — working as designed), opened PR #462, waited out the check-runs race correctly, watched `Run pipeline test` and `Validate content registry` both pass, and merged — `mergedBy: royalte-content-bot`. `main`'s branch protection, required status check, and absence of any bypass were all independently re-confirmed unchanged afterward.

**This closes the certification.** The Content Publishing Engine™ completes a real, unattended, end-to-end publish (registry → PR → required checks → merge → deploy → sitemap/RSS/search-index regeneration → Publication History) with no human step, exactly matching the Board's original success criteria. Five live runs, five real findings, five real fixes — each one the "one complete production-equivalent execution" the Board's certification standard exists to catch, and each one did. Full narrative and evidence for every run: `governance/CONTENT_PUBLISHING_AUTHENTICATION_MODEL.md`.

**Recurring note on "PR #454"**: the directive that authorized this final authentication work referred to "PR #454" as still open and pending merge. PR #454 (the original Content Publishing Engine™ implementation) merged earlier in this program, along with four follow-on fixes (#455, #456, #458, #459, #461) discovered during this same live-certification process. There is no open PR blocking this initiative — the work described as "final merge" in that directive is already on `main` and has now been proven live by Run 5.
