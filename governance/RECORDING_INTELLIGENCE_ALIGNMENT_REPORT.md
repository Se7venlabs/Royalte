...
*Written for both technical and non-technical Executive Board members.*

### Before

Recording Intelligence — the part of Royaltē that analyzes an artist's recordings, matches them across Spotify and MusicBrainz, and certifies which ISRC (the industry-standard recording identifier) belongs to which track — obtained its raw data by reaching directly into Royaltē's internal, pre-processed scan payload (`canonicalForEnrichment`) and pulling out two specific fields itself, buried inside provider-specific paths (`platforms.spotify.details.topTracks`, `platforms.musicbrainz.details.recordings`).

This violated Royaltē's constitutional architecture because that internal payload is not a stable, published contract — it is an implementation detail of the scan engine, expected to change shape as the platform evolves. Royaltē's architecture requires exactly one designated place to translate that internal shape into a stable, domain-specific input (an "Evidence object"), and a separate place to do the actual analysis (an "Intelligence module"). Recording Intelligence did both jobs itself, in one file: it knew things about how the scan engine organizes its internal data that no other part of the system was supposed to need to know, and that dependency was invisible to anyone reviewing the codebase.

It differed from Catalog Intelligence, Backend Intelligence, and Global Music Footprint because those three domains had already been corrected for exactly this issue one day earlier in the same recovery effort (2026-07-20). Recording Intelligence was added to the platform later (Phase 3.7) and was simply missed by that earlier pass — confirmed, not assumed, by the dedicated architectural review this alignment implements (`governance/RECORDING_INTELLIGENCE_ARCHITECTURE_REVIEW.md`, PR #387): no design document, code comment, or prior Board decision anywhere in the repository ever stated Recording Intelligence should be treated differently.

### After

Recording Intelligence now receives one thing: a `Recording Evidence` object, produced by a single new file, `api/_lib/recording-evidence.js`. That object already contains exactly the three fields Recording Intelligence needs — the artist's name, their Spotify top tracks, and their MusicBrainz recordings — in a flat, stable shape that has nothing to do with how the scan engine stores data internally.

`recording-evidence.js` is now the constitutional owner of recording-level evidence because it is the only file in the codebase permitted to know where those fields live inside Royaltē's internal scan payload. If that internal shape changes in the future, exactly one file needs to change to accommodate it — Recording Intelligence's own logic never has to know or care.

Recording Intelligence is now responsible solely for intelligence generation because, with data retrieval moved out, everything left in `lib/recording/recording-intelligence.js` is genuine analysis: matching the same recording across two different providers, resolving conflicting ISRCs, and scoring how confident Royaltē is in each result. It no longer touches, or needs to know about, Royaltē's internal scan-engine plumbing at all.

---

## Architectural Consistency Matrix

The permanent constitutional reference for how every recovered intelligence domain sources its evidence:

| Domain | Evidence Owner | Intelligence Module | Status |
|---|---|---|---|
| Catalog | `api/_lib/catalog-evidence.js` | Catalog Intelligence (`api/_lib/catalog-intelligence.js`) | Complete |
| Backend | `api/_lib/backend-evidence.js` | Backend Intelligence (`api/_lib/backend-intelligence.js`) | Complete |
| Global Music Footprint | `api/_lib/global-footprint-evidence.js` | Global Music Footprint (`api/_lib/global-music-footprint.js`) | Complete |
| Recording | `api/_lib/recording-evidence.js` | Recording Intelligence (`lib/recording/recording-intelligence.js`) | Complete |

Every row follows the identical constitutional shape: one evidence-owning file performing structural relocation only, one intelligence module performing analysis only, one call site wiring them together inside `lib/rie/index.js`. No domain assembler in the codebase reads `canonicalForEnrichment` directly as of this alignment.

---

## CIO Recovery Program Closure

This section formally closes the **CIO Recovery Program** — the effort tracked under `governance/adr/ADR-002-CIO-Scope.md`, addressing the finding (originally "N4") that certain domain assemblers bypassed the Canonical Intelligence Object (CIO) by reading pre-CIO data directly.

**CIO Bypass Findings (ADR-002 / N4):**

- Catalog Intelligence — Resolved (2026-07-20)
- Backend Intelligence — Resolved (2026-07-20)
- Global Music Footprint — Resolved (2026-07-21)
- Recording Intelligence — Resolved (2026-07-21, this alignment)

**Remaining CIO Bypasses: None.** ADR-002 is fully closed — every domain assembler in the codebase reads a dedicated sibling evidence object rather than raw canonical data.

**Scope note, for accuracy of the permanent record:** the CIO Recovery Program (ADR-002 / N4) is a distinct, narrower effort from the broader Normalization Layer Certification (`governance/NORMALIZATION_LAYER_PLATFORM_CERTIFICATION.md`, findings N1-N6). This alignment closes ADR-002/N4 completely. It does **not** close, and was never scoped to close, findings N1 and N2 from that broader certification — the duplicate, discarded computation inside `EvidenceBridge.js` for 7 of 9 providers. Those findings remain real, currently active, and correctly gated behind a separate, still-pending Board architectural decision, `governance/adr/ADR-004-Provider-Merge-Authority.md`. `governance/NORMALIZATION_LAYER_COMPLETION_REPORT.md` (PR #386) certified the Normalization Layer overall as 🟡 Certified with Conditions for exactly this reason, and that overall rating is unchanged by this alignment — only the ADR-002 sub-finding it also tracked is now fully resolved. A future reader of this document should not infer from "CIO Recovery Program: closed" that the Normalization Layer itself has reached 🟢.

---

## Scope Verification

Confirmed, file-by-file, against the actual committed diff (`api/_lib/recording-evidence.js` new — 105 lines; `lib/recording/recording-intelligence.js` modified — 53 lines changed; `lib/rie/index.js` modified — 34 lines changed; 2 certification suites modified; 3 governance documents modified/added):

| Claim | Verified |
|---|---|
| Did not introduce new providers | ✓ — zero new connector, PAL-acquisition, or `EngineProviderRegistry.js` entries in this diff |
| Did not introduce new intelligence | ✓ — `recording-intelligence.js`'s analysis logic (`recording-normalizer.js` → `canonical-recording.js` → `isrc-certification.js` → `recording-confidence.js` → `computeStats`) is untouched; only its input source changed |
| Did not modify CIO | ✓ — `api/_lib/cio-assembler.js`, `api/schema/cio.js` absent from the diff |
| Did not modify CIM | ✓ — `api/schema/canonical-intelligence-model.js` absent from the diff; §8.2.13 `recording` output shape verified byte-identical by certification Suite 04 (CIM Structural Integrity), passing unchanged |
| Did not modify Runtime Context | ✓ — `public/js/runtime-context-mapper.js` absent from the diff |
| Did not modify Mission Control outputs | ✓ — no file under `public/` appears in the diff |
| Did not change scan behaviour | ✓ — PAL acquisition, Path A/Path B normalization, and `canonicalForEnrichment` production are all untouched; the only change is which file reads the finished result |
| Did not change any artist-facing functionality | ✓ — the CIM's persisted `recording` field is structurally and semantically identical before and after; nothing an artist or Mission Control workspace could observe is different |

This confirms the work performed was architectural only.

---

## 1. Phase 1 — Recording Pipeline Audit (verified before coding)
...
**Current contract — closed set:** `artistName`, `spotifyTopTracks`, `musicbrainzRecordings`. This is the complete, current list. Any future addition to this contract requires constitutional ownership review and Executive Board approval before implementation — see "Future Governance" at the end of this report.

---

## 4. Phase 5 — Migration Dependency Elimination (verified, not assumed)
...
## 5. Phase 6 — MusicBrainz Verification
...
**Runtime Context** (`public/js/runtime-context-mapper.js`) and **Mission Control**: not independently re-tested by this session (no dedicated automated suite invoked for either in this battery), but both read `cim.recording`, whose shape is unchanged (§8) — no code path in either was touched.

**New failures introduced by this alignment: zero.**

---

## 8. Contract Verification — CIO / CIM / Runtime Context / Mission Control unchanged
...
Revert the single commit on this branch. All 7 changed files (§2) revert together cleanly — `recording-intelligence.js`'s signature and `lib/rie/index.js`'s call site are interdependent and must move together (consistent with ADR-002's own Migration Strategy note: "any rollback must revert the assembler signature and the call site together, or the pipeline throws"). No persisted-data concern — CIM is rebuilt fresh per scan, so no historical `audit_scans` rows require migration in either direction. No version bump required — this is an internal wiring change, not a shape change to any versioned contract (`AUDIT_RESPONSE_VERSION`, `CIO_VERSION`, `CIM_VERSION` all unaffected).
...
- ✓ Migration-layer *dependency* is no longer undocumented (the mechanism itself is unchanged, correctly — see §4)
- ✓ MusicBrainz recording evidence is preserved, not fabricated, and the reason it's empty is documented (§5)
- ✓ No duplicate normalization exists — confirmed, one relocation, one intelligence pass
- ✓ CIO / CIM / Runtime Context / Mission Control unchanged (§8)
- ✓ Regression suite passes — 1588/1588 on the full certification harness, zero new failures (§7)
...
Recording Evidence (`api/_lib/recording-evidence.js`) is **intentionally minimal**. Its contract — `artistName`, `spotifyTopTracks`, `musicbrainzRecordings` — covers exactly what Recording Intelligence needs today, and nothing more.

This module should not grow speculatively. Any future addition to its contract (a new field, a new provider's recording data, a new relationship type) requires, before implementation:

1. **Constitutional justification** — evidence that the field is genuine, already-normalized recording evidence belonging in the Normalization Layer, not business logic or a convenience shortcut for one caller.
2. **Evidence ownership review** — confirmation that `recording-evidence.js` (and not some other module) is the correct constitutional owner of the new field, consistent with the one-owner-per-field discipline established across `catalog-evidence.js`, `backend-evidence.js`, and `global-footprint-evidence.js`.
3. **Executive Board approval** — no expansion of this contract, or introduction of a comparable evidence object elsewhere, should proceed without an explicit Board decision, following the same evidence-first discipline used throughout the CIO Recovery Program.

This rule exists to prevent the sibling-evidence-object pattern from becoming a new, unbounded catch-all — the same failure mode the pattern itself was built to correct in `canonicalForEnrichment`. The object should only grow when legitimate recording evidence genuinely becomes part of the Normalization Layer, never by default.
