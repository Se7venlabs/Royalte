...
## 14. Phase 1.1 Hotfix — RESOLVED (2026-08-04)

`fireWorkflowDispatch()` in `api/content/decide.js` was receiving `401 Bad Credentials` from GitHub's REST API at Phase 1 closeout. The decision-recording and confirmation-email paths were never affected (§12's `workflow_dispatch` failure path is exactly this scenario and degrades correctly), but ECR-001's "publish immediately after approval" was not restored — approved overdue articles waited for the next scheduled run instead of publishing within seconds.

**Root cause**: `CONTENT_PUBLISHING_PAT` in Vercel held a stale, previously-revoked fine-grained PAT (`github_pat_...`, 93 chars) rather than the current Classic PAT (`ghp_...`, 40 chars) already correct in the GitHub Actions secret of the same name. Not a code defect — `fireWorkflowDispatch()`'s request construction (endpoint, `Bearer` auth scheme, payload) was correct throughout, confirmed by a temporary, never-merged, read-only-then-Board-authorized diagnostic endpoint that isolated the failure to credential validation before any repo/workflow permission was ever reached. The dashboard "update secret" flow did not reliably persist the new value across several attempts (mirroring the same non-persisting pattern seen earlier with `SUPABASE_URL` and `CONTENT_APPROVAL_TOKEN_SECRET`); resolved via `vercel env rm` / `vercel env add` (CLI), independently confirmed via `vercel env ls`'s reported age before any live retest.

**Live-verified, real production content, not simulated**: approving a real article (`metadata-mistakes-killing-royalties`, PR #399's content, previously stuck since 2026-07-23) triggered a new GitHub Actions run within 1 second of the decision being recorded — confirmed via `gh run list` timestamps, not inferred. That run published the article, merged automatically, and updated RSS/search-index/Publication History in the same pass. No code changes were required — the fix was entirely credential rotation.

**Standing item, not blocking, tracked separately in `governance/ROADMAP.md`'s Phase 1.2**: the working Classic PAT authenticates as `Se7venlabs` (the repository owner), not `royalte-content-bot` (the dedicated least-privilege machine-account collaborator the Content Publishing Engine's own authentication model — `governance/CONTENT_PUBLISHING_AUTHENTICATION_MODEL.md` — was built around). Functionally correct today; a deliberate deviation from the original least-privilege design, not yet corrected.
