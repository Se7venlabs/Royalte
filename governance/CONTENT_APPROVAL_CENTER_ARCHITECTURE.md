...
## 14. Known open item at Phase 1 closeout (does not block certification)

`fireWorkflowDispatch()` in `api/content/decide.js` calls GitHub's REST API using `process.env.CONTENT_PUBLISHING_PAT` and, as of Phase 1 closeout, receives `401 Bad Credentials` — confirmed via live Vercel function logs. This does not lose or corrupt any decision (§12's `workflow_dispatch` failure path is exactly this scenario, and it degrades correctly: the decision is recorded, the confirmation email sends, and `sync-approvals.mjs`'s next scheduled/manually-triggered run picks it up regardless). The only effect is that ECR-001's "publish immediately after approval" is not yet restored — approved overdue articles wait for the next run instead of publishing within seconds. Tracked as an immediate Phase 1.1 hotfix, not a Phase 1 defect; root cause and fix to be recorded here once confirmed.
