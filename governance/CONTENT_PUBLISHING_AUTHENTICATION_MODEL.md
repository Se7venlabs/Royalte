...
**Standing recommendation, not yet actioned**: since PAT-based PR creation doesn't depend on `can_approve_pull_request_reviews` at all, that repo-wide setting (§1) is no longer load-bearing for this workflow. Reverting it to `false` would tighten the repo further (no workflow's `GITHUB_TOKEN` would retain PR-create/approve capability). Not reverted yet — flagged for a deliberate follow-up decision rather than bundled into this change.
