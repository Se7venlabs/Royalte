...
**Identity Intelligence™ Phase 3C (PR #301, `041e40f`, tag `identity-intelligence-phase3c-v1.0`, 2026-07-11):** Canonical Identity Provider set expanded from 3 to 5 (apple/spotify/youtube/deezer/tidal). `IDENTITY_PROVIDERS` constant is the sole source of truth; all calculations derive from it. `cio.observations.providers` now initialized for all 5 providers. ISRC-First Verification Strategy™: when scan input carries a verified track ISRC, `discoverSpotifyByIsrc()` fires first (step 1) — no new matching logic. ISRC bridge (step 4) is artist-URL fallback; Apple returns songs ordered by play count (deterministic). Apple Music API limit=25 → limit=20 fixed in 4 call sites. 53 assertions (34 identity-intelligence + 19 identity-wiring).
