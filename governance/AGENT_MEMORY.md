...
| | |
|---|---|
| Most recently completed phase | **Global Music Footprint™ — Canonical Artist Territory Intelligence™** |
| Merged at | `1e7ed72` (PR #428, 2026-07-26) |
| Active direction | **Resolved the Prince Territory Intelligence regression the Board reported.** Root cause: artist-only scans fell back to `extractFirstAlbumId()`, an arbitrary catalog-order pick, traced to architecture drift at commit `584770d` (2026-07-02) predating the Territory Intelligence Engine. Fixed by OR-aggregating across a Best Verified Release™-ranked sample (up to 5 releases) instead — reuses existing Board-governed scoring and the Engine's existing OR-style storefront classification; zero changes to Territory Intelligence Engine™, Runtime Context, or Mission Control (proven via a test calling the real Engine directly). Live-verified against the PR #428 Preview deployment with real Apple/Spotify credentials: Apple's batch `ids=` parameter confirmed, 4-artist live regression (Prince/Dua Lipa/Ada Lea/Fela Kuti) all 91–94% coverage, all 6 Board-named failure states confirmed to degrade honestly (never fabricated `UNAVAILABLE`). Methodology transparency added: the 5-release sample is explicitly classified as a bounded approximation (not complete-catalog truth), carried end-to-end as `territoryMethodology`/`evaluationMethodology` metadata; UI not yet wired to it (out of this phase's scope). |
| Next Board gate | None required — UI surfacing of `evaluationMethodology` and any full-catalog evaluation upgrade would each need their own brief if pursued. |
| Prior phase | ATHENA™ Phase 3B — Executive History™, Timeline™, Memory™, Comparison™ (`f78ce31`, PR #426, 2026-07-25) |
...
Phase lock history: MC v2.0 Sprint 1 `a902da7` / `canonical-registry-sprint1-v1.0`. MC v2.0 Sprint 2 `d10d469`. MC v2.0 Sprint 3 `73a036d`. MC v2.0 Sprint 4 `43761fa`. MC v2.0 Sprint 5 `8c7fb5f`. MC v2.0 Sprint 6 (Canonical Intelligence Domains™) — ✅ COMPLETE — constitutionally ratified architecture implemented during expanded Sprint 1 (`a902da7`); no separate engineering implementation; `canonical-registry-sprint1-v1.0`. MC v2.0 Sprint 7 `8a4aa19`. MC v2.0 Sprint 8 `1317f07`. MC v2.0 Sprint 9 `4047311` / `mission-control-api-v1.0`. Phase 6 `a23788b` / `intelligence-engine-v1.0`. Phase 6.5 `52b1750`. Phase 7 `ec57481`. Phase 7.5 `38ec3be`. Phase 8 `8f00014` / `phase-8-scan-pipeline-wiring-v1.0`. Phase 3.1 (CimAdapter + scan-migration) `77c827a`. Phase 3.2 (One Health Engine) `aca5571`. Phase 3.3 (Apple Production Migration) `584770d`. Phase 3.4 (Product Consumption Cleanup) `8a71df7`. Phase 3.5 (OS v1.0 Certification) `65c5c16` / `royalte-os-v1.0`. Phase 3.6 Spotify `ba4054d`. Phase 3.7 (Recording Intelligence) `2057db6`. Phase 3.8 (MusicBrainz) `b966881`. Phase 3.6 Discogs `aea8095`. Phase 3.6 YouTube `fb44ef5`. Phase 3.6 MLC `67d7fe8`. Phase 3.6 Deezer `ba66b26`. MC-3.2 Health Intelligence™ Executive Assessment `346a2d0`. MC-3.2-ELO Executive Layout Optimization™ v1.0 `83c8804`. MC-3.3 Identity Intelligence™ Executive Passport `654eb52`. MC-3.4 Publishing Intelligence™ Executive Passport + Amendment 1 `2bb1af2`. MC-3.4 Amendment 2 (executive brief typography) `8400134`.
