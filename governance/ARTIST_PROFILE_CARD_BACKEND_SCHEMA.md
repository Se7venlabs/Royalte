...
**Important scope finding:** despite the module name, this object is narrowly a **verification/registration infrastructure signal** — only two services are displayed by Board directive, **MusicBrainz and MLC** (`backend-intelligence.js:74-77`); Discogs/Last.fm feed only the `apisResponding` count, not the `services` list. It has no concept of scan pipeline state, processing errors, or a general data-quality signal. The approved product model's broader scope (system status, data health, processing history, full provider connection rollup, system alerts) is largely **not yet represented** by this object — documented honestly in Section 2, not assumed to exist.

## Major finding: the Digital Twin™ hero is almost entirely static

`public/workspaces/backend-intelligence.html`'s centerpiece — 7 nodes (RELEASE / METADATA / DISTRIBUTION / RIGHTS OWNERSHIP / IDENTITY / REGISTRATIONS / CANONICAL RECORD), their verified/review states, all connection lines, and the full ATHENA drill-down panel content (`backend-intelligence.html:438-642`) — is **static markup and JS**, not bound to `ctx.backendIntelligence` at all. Only four things on the entire page are live-bound:
...
The "Infrastructure Risk" KPI card ("MEDIUM", "2 Issues Detected," lines 392-402) and the per-node "review" states (`bi-node--review` on Rights/Registrations, lines 469, 492) are hardcoded, not computed. This is the same fixture-not-live pattern already found and logged for Catalog Intelligence and Global Music Footprint, but larger in scope — most of this workspace's visual surface has no real data behind it today.
...
|---|---|---|
| Profile Status | No such field anywhere in the assembler | New field — genuinely undefined concept today, needs product definition before engineering scope |
| Processing Status | Nearest analog is scan-write status (`audit_scans`/`scan_snapshots.status`, `api/_lib/persist-os-scan.js:284`) — never exposed through `backendIntelligence` | Surface existing scan status through the assembler — largely a pass-through, not new computation |
...
| Last Successful Sync | The assembler already computes `lastSync` from `canonical.scannedAt` (`backend-intelligence.js:209-211,218`) — but it is **never rendered** in the workspace | UI-binding gap only — the data exists; render it |
...
| Last Scan Date | Exists in the database (`scanned_at`, `persist-os-scan.js:280`) but not surfaced through `backendIntelligence` | Pass-through from existing DB field into the assembler output — same pattern as Last Successful Sync above (likely the same underlying timestamp) |
...
| Processing Errors | No field found anywhere in the assembler | New field |
...
2. PAL owns provider acquisition and raw availability; CIO/CIM owns normalization; Royaltē System (this assembler) owns status calculations, data health, and processing state; the Artist Profile Card represents the result — it does not compute it.
...
- ✅ Major finding documented, not fixed: most of the Digital Twin™ hero and the Infrastructure Risk KPI are static, not data-bound — to be logged as separate engineering tasks
