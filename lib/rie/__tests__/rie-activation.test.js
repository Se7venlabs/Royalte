...
await testAsync('16. Same Evidence Contract produces identical intelligence (only metadata differs)', async () => {
  ...
  // globalFootprint.territoryGeneratedAt (Board Phase 1, IC-1, 2026-07-27)
  // is the Territory Intelligence Engine's own real-wall-clock generation
  // timestamp -- the Engine's signature is Board-locked and does not
  // accept an injectable clock, so this one field is expected to vary
  // between the two calls even under a fixed runRIE() clock, exactly like
  // scanAuthority.certificationId below. Excluded from the strict
  // deepEqual for that reason; everything else must still match exactly.
  const { territoryGeneratedAt: gen1, ...gf1Rest } = cim1.globalFootprint;
  const { territoryGeneratedAt: gen2, ...gf2Rest } = cim2.globalFootprint;
  assert.deepEqual(gf1Rest, gf2Rest);
  assert.ok(typeof gen1 === 'string' && !isNaN(new Date(gen1).getTime()));
  assert.ok(typeof gen2 === 'string' && !isNaN(new Date(gen2).getTime()));
