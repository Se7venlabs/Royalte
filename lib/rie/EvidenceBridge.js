...
// Translate Apple TRACKS contract payload into
// platforms.appleMusic.details.tracks[] -- the artist's own official Apple
// Music song catalog, each carrying Apple's first-party isrc attribute when
// assigned. Sole evidence source for ISRC Intelligence™ v1
// (api/_lib/isrc-intelligence.js, Board directive 2026-07-21).
// Apple Song resource: { id, type: 'songs', attributes: { name, isrc, ... } }
function translateAppleTracks(packages, canonical) {
  // Scoped by provider (not plain findFirst) -- unlike ALBUMS, TRACKS is
  // also supplied by Spotify/MusicBrainz/Deezer/Last.fm and an unscoped
  // lookup could silently bridge the wrong provider's catalog here.
  const pkg = findFirstByProvider(packages, 'apple_music', Capability.TRACKS);
  if (!pkg) return;
  ...
  canonical.platforms.appleMusic.details.tracks = data
    .filter(n => n?.type === 'songs')
    .map(n => ({
      id:    n.id                  ?? null,
      name:  n.attributes?.name    ?? null,
      isrc:  n.attributes?.isrc    ?? null,
      albumName: n.attributes?.albumName ?? null,
    }));
}

// Translate VIDEOS contract payload into platforms.appleMusic.details.videos[].
// Media PAL Expansion™ acquired this data (search scoped to the
// music-videos resource type, per AppleMusicConnector.js's
// #fetchArtistMusicVideos comment -- Apple's Catalog API has no artist
// relationship/view for music videos) but never translated it into
// canonical, per Media PAL Expansion™'s explicit scope boundary
// (governance/MEDIA_PAL_EXPANSION_COMPLETION_REPORT.md §6). Wired here
// for Media Intelligence™ (Board directive 2026-07-22), following the
// identical pattern translateAudioDBVideos() already establishes --
// raw fields preserved intact, no interpretation.
// Payload shape (search response): { results: { "music-videos": { data: [
//   { id, type: "music-videos", attributes: { name, artistName,
//     releaseDate, durationInMillis, artwork, genreNames, isrc,
//     contentRating } } ] } } }
function translateAppleVideos(packages, canonical) {
  // Scoped by provider, same reasoning as translateAppleTracks above --
  // YouTube and TheAudioDB also declare Capability.VIDEOS.
  const pkg = findFirstByProvider(packages, 'apple_music', Capability.VIDEOS);
  if (!pkg) return;

  const data = pkg.contract.payload?.results?.['music-videos']?.data;
  if (!Array.isArray(data)) return;
  ...
  const videos = data
    .filter(n => n?.type === 'music-videos')
    .map(n => ({
      id:               n.id                          ?? null,
      name:             n.attributes?.name             ?? null,
      artistName:       n.attributes?.artistName       ?? null,
      releaseDate:      n.attributes?.releaseDate      ?? null,
      durationInMillis: n.attributes?.durationInMillis ?? null,
      artwork:          n.attributes?.artwork          ?? null,
      genreNames:       n.attributes?.genreNames       ?? [],
      isrc:             n.attributes?.isrc             ?? null,
      contentRating:    n.attributes?.contentRating    ?? null,
    }));

  canonical.platforms.appleMusic.details.videos = videos;
  canonical.platforms.appleMusic.videoCount = videos.length;
}
...
// Translate VIDEOS contract payload into platforms.youtube.details.videos[].
// Media PAL Expansion™ acquired this data (videos.list?part=snippet,
// contentDetails,statistics,status against the channel's uploads playlist)
// but never translated it into canonical, per Media PAL Expansion™'s
// explicit scope boundary (governance/MEDIA_PAL_EXPANSION_COMPLETION_REPORT.md
// §6). Wired here for Media Intelligence™ (Board directive 2026-07-22),
// following the identical pattern translateAudioDBVideos() already
// establishes -- raw fields preserved intact, no interpretation. Duration
// is preserved as the raw ISO 8601 string; any Shorts (<=60s) inference
// belongs to the consuming intelligence domain, not this structural bridge.
// Payload shape: { items: [ { id, snippet: {title, description,
//   publishedAt, thumbnails, tags}, contentDetails: {duration, definition},
//   statistics: {viewCount, likeCount, commentCount}, status: {privacyStatus} } ] }
function translateYouTubeVideos(packages, canonical) {
  const pkg = findFirstByProvider(packages, YOUTUBE_PROVIDER, Capability.VIDEOS);
  if (!pkg) return;
  ...
  canonical.platforms.youtube.details.videos = videos;
  canonical.platforms.youtube.details.publicVideoCount =
    videos.filter(v => v.privacyStatus === 'public').length;
}
...
export function bridgeToCanonical(evidencePackages) {
  ...
  try { translateAppleTracks(evidencePackages, canonical);    } catch { /* never throw */ }
  try { translateAppleVideos(evidencePackages, canonical);    } catch { /* never throw */ }
  ...
  try { translateYouTubeVideos(evidencePackages, canonical);            } catch { /* never throw */ }
