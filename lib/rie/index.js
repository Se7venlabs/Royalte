...
export function assembleCIM({
  ...
  // unified here into one constitutional object. Phase 5 retires the legacy
  // split-shape paths.
  if (healthIntelligence || healthScore) {
    cim.health = {
      score:          healthIntelligence?.score          ?? healthScore?.overallScore ?? null,
      grade:          healthScore?.overallGrade          ?? null,
      status:         healthIntelligence?.status         ?? null,
      categoryScores: healthScore?.categoryScores        ?? null,
      drivers:        healthIntelligence?.drivers        ?? null,
...
// ── runRIE ────────────────────────────────────────────────────────
//
// The sole RIE entrypoint. Accepts normalized provider evidence,
// runs the full assembly chain, maps to the 12-object CIM, and
// emits a certified, deep-frozen Canonical Intelligence Model.
//
// evidence:
//   canonicalForEnrichment  AuditResponse  normalized provider evidence
//   publishingWorks         PublishingWork[] | null  pre-resolved works
//   publishingSourceObservations  object | null
//
// options:
//   artistName  string    (falls back to evidence.canonicalForEnrichment.subject.artistName)
...
export async function runRIE(evidence, options = {}) {
  ...
  // Phase 2: evidence will arrive as raw PAL adapter outputs.
  // Validate presence of the minimum required fields.
  if (!canonicalForEnrichment || typeof canonicalForEnrichment !== 'object') {
    console.error('[rie] runRIE called with missing canonicalForEnrichment — returning empty CIM');
    const empty = emptyCIM();
    empty.scanAuthority = { _cimVersion: CIM_VERSION, generatedAt: now(), error: 'missing_evidence' };
    return certifyCIM(empty, { generatedAt: now() });
  }
  ...
  // (In Phase 2, this receives raw provider adapter outputs.)
