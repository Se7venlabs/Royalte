...
// Media Intelligence™ — Board directive 2026-07-22. Same sibling-evidence-
// object pattern as Catalog/Backend/Global Music Footprint/Recording above.
import { assembleMediaIntelligence }     from '../../api/_lib/media-intelligence.js';
import { assembleMediaEvidence }         from '../../api/_lib/media-evidence.js';
...
export function assembleCIM({
  ...
  // §8.2.14 media{} — Media Intelligence™ (Board Extension, 2026-07-22)
  cim.media = mediaIntelligence ?? null;
...
export async function runRIE(evidence, options = {}) {
  ...
  // Media Intelligence™ — Board directive 2026-07-22 (KPI Discovery /
  // Implementation authorization). Sibling-evidence-object pattern, same as
  // every other domain assembler above -- reads only the fields
  // media-evidence.js extracts, never canonicalForEnrichment directly.
  const mediaEvidence = assembleMediaEvidence(canonicalForEnrichment);
  let mediaIntelligence = null;
  try { mediaIntelligence = assembleMediaIntelligence(mediaEvidence, Date.now()); }
  catch (e) { console.error('[rie] Media Intelligence failed:', e.message); }
