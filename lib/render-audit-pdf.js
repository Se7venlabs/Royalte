...
function buildActions(canonical) {
  ...
  // 2026-06-07 Canonical Identity Architecture rule #7 — Royaltē never
  // exposes source-platform names in user-facing copy. Items reference
  // Royaltē Intelligence categories instead of specific DSPs.
  if (apple.availability !== 'VERIFIED') {
    items.push({
      observation: 'Streaming Presence could not be independently verified from reviewed sources.',
      whyMatters: 'Major streaming ecosystem presence is an important signal for catalog verification and royalty visibility.',
      action: 'Confirm your artist profiles across major streaming services are properly claimed and maintained.',
      expectedOutcome: 'Additional streaming ecosystem verification was confirmed, increasing catalog confidence and metadata validation.',
      effort: '15 Minutes',
      priority: 'High',
    });
  }
  if (spotify.availability !== 'VERIFIED') {
    items.push({
      observation: 'Audience and platform verification signals could not be independently confirmed.',
      whyMatters: 'Streaming audience metrics provide cross-reference signals for catalog reach and listener engagement.',
      action: 'Confirm your artist profiles across major streaming services are properly claimed and maintained.',
      expectedOutcome: 'Audience verification signals were confirmed; catalog reach picture strengthened.',
      effort: '15 Minutes',
      priority: 'High',
    });
  }
  if (youtube.availability !== 'VERIFIED') {
    items.push({
      observation: 'Video Ecosystem presence could not be independently verified from reviewed sources.',
      whyMatters: 'Video ecosystem visibility supports catalog discovery and rights-monitoring across user-generated content.',
      action: 'Confirm your official artist channel and that your distributor has delivered your catalog to the video streaming ecosystem.',
      expectedOutcome: 'Video ecosystem verification was confirmed; discovery and rights-monitoring signals become trackable.',
      effort: '15 Minutes',
      priority: 'Medium',
    });
  }
  ...
  if (!sfa || typeof sfa !== 'object') {
    items.push({
      ...
      action: 'Run a new scan once Royaltē\'s reviewed territory sources have propagated, or contact your distributor to confirm worldwide release.',
...
function renderUnverifiedHtml(canonical, drivers) {
  ...
  // 2026-06-07 Canonical Identity Architecture rule #7 — categorical
  // Royaltē Intelligence labels in place of source-platform names.
  for (const [key, label] of [['appleMusic', 'Streaming Presence'], ['spotify', 'Audience Verification'], ['youtube', 'Video Ecosystem'], ['musicbrainz', 'Reference Metadata']]) {
...
function verifiedPlatformsList(canonical) {
  ...
  // 2026-06-07 Canonical Identity Architecture rule #7 — categorical
  // Royaltē Intelligence labels in place of source-platform names.
  if (p.appleMusic?.availability  === 'VERIFIED') out.push('Streaming Presence');
  if (p.spotify?.availability     === 'VERIFIED') out.push('Audience Verification');
  if (p.youtube?.availability     === 'VERIFIED') out.push('Video Ecosystem');
  if (p.musicbrainz?.availability === 'VERIFIED') out.push('Reference Metadata');
  return out;
}
