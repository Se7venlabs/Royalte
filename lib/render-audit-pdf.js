...
function scoreToStatus(score) {
  ...
  if (s >= 90) return { label: 'Excellent', cls: 'excellent' };
  if (s >= 75) return { label: 'Strong',    cls: 'strong'    };
  if (s >= 60) return { label: 'Moderate',  cls: 'moderate'  };
...
function buildTopStrengthsHtml(topStrengths, hiStrengths) {
  const items = [
    ...( Array.isArray(topStrengths) ? topStrengths : []),
    ...( Array.isArray(hiStrengths)  ? hiStrengths  : []),
  ].slice(0, 5);
...
function buildTopOpportunitiesHtml(topOpportunities, concerns) {
  const items = [
    ...( Array.isArray(topOpportunities) ? topOpportunities : []),
...
function buildHealthBreakdownRowsHtml(domains) {
  return domains.map(({ name, score }) => domainRow(name, score, scoreToStatus(score))).join('');
}
...
function buildCatalogStatBoxesHtml(ci, gmf, pi) {
  ...
  const works    = pi?.registrations ? Object.keys(pi.registrations).length : 0;
...
function buildBackendCardsHtml(ii, pi, ci, gmf, bi, mi, healthScore) {
  const domains = [
    {
      name: 'Identity',
  ...
  return domains.map(({ name, score, recs }) => {
...
function buildKeyTakeawaysHtml(executiveBrief, royalteAI, hi) {
  const items = [];
  ...
  if (Array.isArray(hi?.concerns)) {
    for (const c of hi.concerns.slice(0, 2)) items.push(c);
  }
...
function buildIntelligenceStatusHtml(ii, pi, ci, bi, mi, royalteAI) {
  const engines = [
    { name: 'Identity Intelligence™',    state: (ii?.verifiedProviders || 0) > 0 ? 'active' : 'partial' },
    { name: 'Publishing Intelligence™',  state: (pi?.coverage || 0) > 0 ? 'active' : 'limited'  },
    { name: 'Catalog Intelligence™',     state: (ci?.albums || 0) > 0 ? 'active' : 'limited'    },
    { name: 'Backend Intelligence™',     state: (bi?.connectedCount || 0) > 0 ? 'active' : 'limited' },
...
function buildTerritoryLabel(gmf, payload) {
  const apple = payload?.platforms?.appleMusic?.details?.storefrontAvailability;
  const count = gmf?.territoriesAvailable;

  // Prefer Apple Music storefront count when available — it's verified
  if (apple && typeof apple === 'object') {
    const verifiedCount = Object.values(apple).filter((v) => Array.isArray(v?.available) && v.available.length > 0).length;
    if (verifiedCount > 0) {
      return { big: `${verifiedCount}`, detail: 'Apple Music Territories Verified', short: String(verifiedCount), shortDetail: 'Apple Music Territories' };
    }
  }

  if (typeof count === 'number' && count > 0) {
    return { big: String(count), detail: 'Countries Verified', short: String(count), shortDetail: 'Countries' };
  }
...
// ── Catalog year range from CIO albums ────────────────────────────────────────
function catalogYearRange(payload) {
  try {
    const albums = payload?.platforms?.appleMusic?.details?.albums;
    if (!Array.isArray(albums) || !albums.length) return { first: '—', latest: '—', fillPct: 80 };
    const years = albums
      .map((a) => a?.releaseDate ? new Date(a.releaseDate).getFullYear() : null)
      .filter((y) => y && y > 1900 && y <= new Date().getFullYear());
    if (!years.length) return { first: '—', latest: '—', fillPct: 80 };
    const first  = Math.min(...years);
    const latest = Math.max(...years);
    const fillPct = latest > first ? Math.min(100, Math.round(((latest - first) / 20) * 80) + 20) : 80;
    return { first: String(first), latest: String(latest), fillPct };
  } catch { return { first: '—', latest: '—', fillPct: 80 }; }
}

// ── Publishing coverage → status pill ────────────────────────────────────────
function publishingStatusPill(pi) {
  const cov = typeof pi?.coverage === 'number' ? pi.coverage : 0;
  if (cov >= 75) return { label: 'Verified',    cls: 'verified'   };
  if (cov >= 40) return { label: 'Partial',     cls: 'partial'    };
  if (cov === 0 && pi?.supportedSources?.includes('mlc')) {
    return { label: 'Unavailable', cls: 'unavailable' };
  }
  return { label: 'Unconfirmed', cls: 'partial' };
}
...
function buildPlaceholders(scanId, payload) {
  ...
  const healthBand   = scoreToStatus(overallScore);
  ...
  // Source: Existing Health Score™ only (constitutional requirement).
  // Monitoring + Rights use healthIntelligence informational contributor scores.
  const domainData = [
    { name: 'Identity',   score: phaseIdentityScore   },
    { name: 'Metadata',   score: phaseMetadataScore   },
    { name: 'Publishing', score: phasePublishingScore  },
    { name: 'Rights',     score: hiPublishingScore     },
    { name: 'Collection', score: phaseCoverageScore    },
    { name: 'Monitoring', score: hiMonitoringScore     },
    { name: 'Catalog',    score: phaseCatalogScore     },
  ];

  // ── Territory labels ─────────────────────────────────────────────────────
  const terrInfo    = buildTerritoryLabel(globalMusicFootprint, p);
  ...
  // ── Catalog ──────────────────────────────────────────────────────────────
  const yearRange       = catalogYearRange(p);
  ...
  // ── Publishing ───────────────────────────────────────────────────────────
  const pubPill    = publishingStatusPill(publishingIntelligence);
  const pubCoverage = typeof publishingIntelligence.coverage === 'number'
    ? publishingIntelligence.coverage : 0;
  ...
  const hiStrengths      = Array.isArray(healthIntelligence.strengths)    ? healthIntelligence.strengths    : [];
  const hiConcerns       = Array.isArray(healthIntelligence.concerns)     ? healthIntelligence.concerns     : [];
  ...
  return {
    ...
    top_strengths_html:     buildTopStrengthsHtml(topStrengths, hiStrengths),
    top_opportunities_html: buildTopOpportunitiesHtml(topOpportunities, hiConcerns),
    ...
    catalog_first_year:  esc(yearRange.first),
    catalog_latest_year: esc(yearRange.latest),
    timeline_fill_pct:   String(yearRange.fillPct),
    catalog_confidence:  esc(catalogIntelligence.confidence || 'Partial'),
    catalog_confidence_cls: esc(catConfidenceCls),
    publishing_coverage: String(pubCoverage),
    ...
    key_takeaways_html:      buildKeyTakeawaysHtml(executiveBrief, royalteAI, healthIntelligence),
    ...
    intelligence_status_html: buildIntelligenceStatusHtml(
      identityIntelligence, publishingIntelligence, catalogIntelligence,
      backendIntelligence, monitoringIntelligence, royalteAI
    ),
