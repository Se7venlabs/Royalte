...
export async function fetchMlcWorksByArtist(artistName, options = {}) {
  ...
  if (typeof bearer !== 'string' || bearer === '') {
    console.log('[mlc] Stage 4 — token extract: idToken absent in response → ERROR (keys=%s)',
  ...
  // Never log the token value — log presence and expiry only.
  const expiresIn = tokenJson?.expiresIn ?? null;
  console.log('[mlc] Stage 4 — token acquired: idToken=present expiresIn=%s tokenType=%s',
    expiresIn, tokenJson?.tokenType ?? 'unknown');
  ...
  console.log('[mlc] Stage 5 — search: artistName="%s" endpoint=%s', artistName, searchEndpoint);
  let searchResp;
  try {
    ...
    console.log('[mlc] Stage 5 — search: transport error: %s → ERROR', err?.message || err);
    return {
  ...
  console.log('[mlc] Stage 6 — search: status=%d ok=%s', searchResp.status, searchResp.ok);
  ...
  if (!searchResp.ok) {
    if (searchResp.status === 401 || searchResp.status === 403) {
      console.log('[mlc] Stage 6 — search: bearer rejected (HTTP %d) → AUTH_UNAVAILABLE', searchResp.status);
      return { observation: MLC_OBSERVATION_AUTH_UNAVAILABLE, rawWorks: [] };
    }
    console.log('[mlc] Stage 6 — search: unexpected HTTP %d → ERROR', searchResp.status);
    return {
  ...
  if (rawWorks.length === 0) {
    console.log('[mlc] Stage 7 — result: 0 works returned → NOT_FOUND');
    return {
      observation: { availability: 'NOT_FOUND', details: null },
      rawWorks:    [],
    };
