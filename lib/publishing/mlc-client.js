...
export async function fetchMlcWorksByArtist(artistName, options = {}) {
  ...
  // Count unique writer identities across all works (not the sum of all writer
  // entries per work, which would always exceed the unique-IPI count and falsely
  // trigger deriveWriterIpi's ACTION_REQUIRED branch in publishing-intelligence.js).
  // Key: IPI number when present; otherwise firstName|lastName concatenation.
  const uniqueWriterKeys = new Set();
  for (const w of rawWorks) {
    if (!Array.isArray(w?.writers)) continue;
    for (const wr of w.writers) {
      const ipi   = (typeof wr?.writerIPI        === 'string' ? wr.writerIPI.trim()        : '') || '';
      const first = (typeof wr?.writerFirstName   === 'string' ? wr.writerFirstName.trim()  : '') || '';
      const last  = (typeof wr?.writerLastName    === 'string' ? wr.writerLastName.trim()   : '') || '';
      const key   = ipi || `${first}|${last}`;
      if (key && key !== '|') uniqueWriterKeys.add(key);
    }
  }
  const writerCount = uniqueWriterKeys.size;
