...
//
//    Stage 1 — Token exchange (all MLC endpoints require auth):
//      POST /oauth/token  { username, password } | { refreshToken }
//      → { idToken, … }
//
//    Stage 2 — Recording discovery:
//      POST /search/recordings   { "artist": artistName }
//      Authorization: Bearer <idToken>
//      ← [ { mlcsongCode, title, isrc, artist, labels, id }, … ]
//
//    Stage 3 — Work detail lookup (capped at WORK_CAP):
//      GET /work/id/{mlcSongCode}   Authorization: Bearer <idToken>
//      ← { primaryTitle, iswc, mlcSongCode, writers[…], publishers[…], … }
//
//    /search/recordings accepts a plain artist name string — no title
//    required, no writer-name splitting. /work/id returns the full
//    composition record with ISWC, writer IPIs, and publisher shares.
//
//    Note on auth: the MLC API documentation describes /search/recordings
//    as unauthenticated, but the live endpoint returns 401 without a
//    bearer token (confirmed 2026-06-30 via publishing-trace). Token
//    exchange runs first so the same bearer covers both calls.
//
...
//
//  Bearer-field selection locked to idToken:
...
export async function fetchMlcWorksByArtist(artistName, options = {}) {
  ...
  // All MLC endpoints require auth (confirmed 2026-06-30: /search/recordings
  // returns 401 without a bearer token despite API docs saying otherwise).
  // Token exchange runs first so the same bearer covers all downstream calls.
  ...
  console.log('[mlc] Stage 3 — token acquired: idToken=present expiresIn=%s tokenType=%s',
    tokenJson?.expiresIn ?? null, tokenJson?.tokenType ?? 'unknown');

  // ── Stage 2: Recording discovery ─────────────────────────────────
  // POST /search/recordings { "artist": artistName }
  // Authorization: Bearer <idToken>
  // Returns one record per recording with mlcSongCode linking to the
  // underlying composition. No title required, no writer-name splitting.
  console.log('[mlc] Stage 4 — recordings discovery: artistName="%s" endpoint=%s', artistName, recordingsEndpoint);
  ...
  try {
    ...
    console.log('[mlc] Stage 4 — recordings: transport error: %s → ERROR', err?.message || err);
    return {
  ...
  console.log('[mlc] Stage 5 — recordings: status=%d ok=%s', recordingsResp.status, recordingsResp.ok);

  if (!recordingsResp.ok) {
    if (recordingsResp.status === 401 || recordingsResp.status === 403) {
      console.log('[mlc] Stage 5 — recordings: bearer rejected (HTTP %d) → AUTH_UNAVAILABLE', recordingsResp.status);
      return { observation: MLC_OBSERVATION_AUTH_UNAVAILABLE, rawWorks: [] };
    }
    console.log('[mlc] Stage 5 — recordings: HTTP %d → ERROR', recordingsResp.status);
    return {
  ...
  console.log('[mlc] Stage 5 — recordings: count=%d', recordings.length);

  if (recordings.length === 0) {
    console.log('[mlc] Stage 5 — recordings: 0 results → NOT_FOUND');
  ...
  if (songCodeSet.size === 0) {
    console.log('[mlc] Stage 5 — recordings: no mlcSongCode values in response → NOT_FOUND');
    return { observation: { availability: 'NOT_FOUND', details: null }, rawWorks: [] };
  }
  console.log('[mlc] Stage 5 — recordings: %d unique mlcSongCode values (cap=%d)', songCodeSet.size, WORK_CAP);

  // ── Stage 3: Work detail lookup (parallel, allSettled) ────────────
  // GET /work/id/{mlcSongCode}  Authorization: Bearer <idToken>
