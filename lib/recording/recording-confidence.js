...
//
// Confidence Dimensions (weights are Board-ratified — see recording-confidence-policy.js):
//
//   ISRC certification  — highest weight; ISRC is the industry's primary
//                         recording identifier; agreement across sources is
//                         the strongest signal of royalty and rights accuracy.
//
//   Provider coverage   — more independent sources → higher trust.
//
//   Title agreement     — normalized titles from multiple sources converging
//                         on the same key strengthens identity confidence.
//
//   Artist agreement    — reserved; activates when Apple and Spotify artist
//                         names can be compared. Currently contributes the
//                         full weight as a base value (no known conflicts).
//
// This module owns calculation only.
// RECORDING_CONFIDENCE_POLICY owns all Board-ratified constants.
//
// Pure. No I/O. Never throws.

import { normalizeTitle }              from './title-normalizer.js';
import { RECORDING_CONFIDENCE_POLICY } from './recording-confidence-policy.js';

// Destructure Board-ratified weights at module load — fail fast if policy is absent.
const { isrc: W_ISRC, provider: W_PROVIDER, title: W_TITLE, artist: W_ARTIST }
  = RECORDING_CONFIDENCE_POLICY.weights;
