...
//
// Input shape (native MusicBrainz recording response):
//   [{ id, title, isrcs: string[], length }]
//
// isrcs[] is MB's native multi-ISRC format. We take the first valid ISRC.
// Provider knowledge (isrcs[] → isrc) stays here, not in Recording Intelligence.
export function normalizeMusicBrainzRecordings(recordings = []) {
  ...
  return recordings
    ...
    .map(r => {
      const isrcList = Array.isArray(r.isrcs) ? r.isrcs : (r.isrc ? [r.isrc] : []);
      const isrc     = isrcList.map(validIsrc).find(Boolean) ?? null;
