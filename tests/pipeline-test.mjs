...
// TIDAL can produce NOT_FOUND when credentials are absent from the fixture environment.
// All other platforms must not leak AUTH_UNAVAILABLE.
const _nonTidalPlatforms = Object.entries(canonicalAppleOnly.platforms).filter(([k]) => k !== 'tidal');
assert(!_nonTidalPlatforms.some(([,v]) => v?.availability === 'AUTH_UNAVAILABLE'), 'apple canonical contains no AUTH_UNAVAILABLE leakage (tidal excluded — credential-gated)');
